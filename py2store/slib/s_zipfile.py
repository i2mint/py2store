...
# TODO: Writing items to zips doesn't seem to compress them. Look into this.
# TODO: Do system call for deletion (e.g. zip -d filename.zip __MACOSX/\*)
...
- zips are weird, and the zipfile package, which follows the zip protocol is as well.
    For example, there's no (as of today) delete functionality in zipfile, so your `Persister`
    won't be a full persister.
-  The most weirdest: You can have duplicate keys with `zipfile`. Yeah, that's right. It'll just go on
    ...
    But fret not, the `ZipStore` has a "natural" behavior: It won't allow you to create duplicates.
    On the other hand, it won't allow you to overwrite the value of an existing key either (remember, we
    can't delete!)
- If you write items individually in a zip file, you don't really get any ccompression.

Also, since `ZipStore` can write to a zip, it's read functionality is not going to assume static data,
and cache things, as your favorite zip readers did.
This, and the acrobatics need to disguise the weird `zipfile` into something more... key-value natural,
makes for a not so efficient store, out of the box.

We advise using one of the zip readers if all you need to do is read, or sub-classing or
 wrapping `ZipStore` with caching layers if it is appropriate to you.
...
```
z = ZipStore(f, allow_overwrites=False)
assert list(z) == []
```
...
assert list(z.values()) == [b'bar', b'eggs', b'world']
```
