...
class PyCodeReader(RelPathFileStringReader):
    @wraps(FileStringReader.__init__)
    def __init__(self, rootdir, *args, **kwargs):
        parent_dir, dirname = _root_dir_and_name(rootdir)
        rootdir = os.path.join(parent_dir, dirname)
        super().__init__(rootdir, *args, **kwargs)


def mk_code_navigator(root, filepath_filt=only_py_ext, on_error=DFLT_ON_ERROR):
    """
    ...
    pycode, root = mk_pycode_obj(root, filepath_filt)

    for k, code_str in pycode.items():
        try:
            yield extract_info(root, k, code_str)
...
def extract_info(root, k, code_str):
    objs = get_objs(root, k)
    return {
        "filepath": k,
...
def modules_info_df(
    ...
    # mk_code_navigator was modules_info_gen here:
    d = list(mk_code_navigator(root, filepath_filt, on_error=on_error))
